 ...
 * @version     1.0
 *
 * This class is deeply based on RainTPL 2.6.5 by Federico Ulfo. Learn more
 * on Rain Team projects: http://www.raintm.com/
 * on RainTPL:            http://www.raintpl.com/
 *
 * This library is distributed under GNU/LGPL 3 License
 *
 */
class Hydrant {
  ...
  /**
   * Cache directory. Is the directory where Hydrant will compile the template and save the cache
   *
   * @var string
   */
  ...
  /**
   * Template base URL. Hydrant will add this URL to the relative paths of element selected in $path_replace_list.
   *
   * @var string
   */
  ...
  /**
   * Is the array where Hydrant keep the variables assigned
   *
   * @var array
   */
  public $var = array();
  ...
  /**
   * Configure the settings of Hydrant
   *
   */
  ...
  protected function check_template( $tpl_name ){
    ...
    if( !isset($this->tpl['checked']) ){
      ...
      if( self::$check_template_update && !file_exists( $this->tpl['tpl_filename'] ) ){
        $e = new Hydrant_NotFoundException( 'Template '. $tpl_basename .' not found!' );
        throw $e->setTemplateFile($this->tpl['tpl_filename']);
      }

      // file doesn't exsist, or the template was updated, Hydrant will compile the template
  ...
  */
  ...
  protected function compileFile( $tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename ){
    ...
    if( !is_writable( $cache_dir ) )
        throw new Hydrant_Exception ('Cache directory ' . $cache_dir . 'doesn\'t have write permission. Set write permission or set RAINTPL_CHECK_TEMPLATE_UPDATE to false. More details on http://www.raintpl.com/Documentation/Documentation-for-PHP-developers/Configuration/');
  ...
  protected function compileCode( $parsed_code ){
    ...
    while( $html = array_shift( $parsed_code ) ){
      ...
      elseif( preg_match( '/\{include="([^"]*)"(?: cache="([^"]*)"){0,1}\}/', $html, $code ) ){
        ...
        if( isset($code[ 2 ]) )
          //dynamic include
          $compiled_code .= '<?php $tpl = new Hydrant;' .
    ...
    if( $open_if > 0 ) {
      $e = new Hydrant_SyntaxException('Error! You need to close an {if} tag in ' . $this->tpl['tpl_filename'] . ' template');
      throw $e->setTemplateFile($this->tpl['tpl_filename']);
    }
    return $compiled_code;
  }
  ...
  protected function function_check( $code ){
    ...
    if( count(self::$black_list) && preg_match( $preg, $code, $match ) ){
      ...
      $e = new Hydrant_SyntaxException('Unallowed syntax in ' . $this->tpl['tpl_filename'] . ' template');
      throw $e->setTemplateFile($this->tpl['tpl_filename'])
        ->setTag($code)
   ...
   *
   * @param Hydrant_Exception $e
   * @return string
   */
  protected function printDebug(Hydrant_Exception $e){
    if (!self::$debug) {
      throw $e;
    }
    ...
    if ($e instanceof Hydrant_SyntaxException) {
...
/**
* Basic Hydrant tpl exception.
*/
class Hydrant_Exception extends Exception{
  /**
...
*/
class Hydrant_NotFoundException extends Hydrant_Exception{
}

/**
* Exception thrown when syntax error occurs.
*/
class Hydrant_SyntaxException extends Hydrant_Exception{
  /**
   ...
   * @return Hydrant_SyntaxException
   */
